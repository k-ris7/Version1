...
namespace CapaPresentacion
{
    public partial class frmIndicadorProducto : Form
    {
        ...
        private void LlenarGrid(String name) {
            try
            {
                ...
                for (int i = 0; i < Lista.Count; i++)
                {
                    ...
                    else if (verificarStockPromedio(Lista, i))
                    {
                    ...
                    else if (verificarStocOptimo( Lista, i))
                    {
        ...
        bool verificarStockCero(List<entProducto> Lista, int i)
        {
            return Lista[i].Stock_Prod >= 0 && Lista[i].Stock_Prod <= Lista[i].StockMin_Prod;
        }

        bool verificarStockPromedio(List<entProducto> Lista, int i)
        {
            return Lista[i].Stock_Prod > Lista[i].StockMin_Prod && Lista[i].Stock_Prod <= Lista[i].StockProm_Prod;
        }

        bool verificarStocOptimo(List<entProducto> Lista, int i)
        {
            return Lista[i].Stock_Prod > Lista[i].StockProm_Prod;
        }
